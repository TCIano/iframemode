...
| 你的任务                  | 至少读这些（起点，非白名单）                                                       |
| ------------------------- | ---------------------------------------------------------------------------------- |
| 首次接项目 / 不熟悉规则   | 01-对话规范 → 02-命名规则 → 03-代码约束 → 技术栈/vue-ant-design → 技术栈/vue-pinia |
...
> **Agent 原则：** 路由表是保底的"至少读这些"。实际执行时按「你实际创建/修改了哪些文件」动态判断——动了 `src/api/` 就要读 06-api设计规范，动了 `src/components/` 就要读 07-组件设计规范，输出完代码必须跑 05-自检清单。宁可多读一个不相关的，也不要漏读一个相关的。
